...
assert.ok(pageSource.includes('taskClaims'), '任务页应维护循环任务领取关系')
assert.ok(pageSource.includes('startTaskClaim'), '循环任务应调用开始领取接口')
assert.ok(pageSource.includes('stopTaskClaim'), '循环任务应调用停止领取接口')
assert.ok(pageSource.includes('停止领取'), '任务页应提供停止领取按钮')
