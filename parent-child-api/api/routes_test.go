...
func TestRegisterRoutesTaskStartClaimRejectsInvalidJSON(t *testing.T) {
	const jwtSecret = "test-secret"
	token, err := ux.JwtUtil.SignAuthToken(ux.AuthTokenClaims{UserId: 1}, jwtSecret, time.Hour)
	if err != nil {
		t.Fatalf("SignAuthToken error = %v", err)
	...
	req := httptest.NewRequest(http.MethodPost, "/api/task/startClaim", strings.NewReader("{"))
	req.Header.Set("Authorization", "Bearer "+token)
	res := httptest.NewRecorder()
...
func TestRegisterRoutesTaskStopClaimRejectsInvalidJSON(t *testing.T) {
	const jwtSecret = "test-secret"
	token, err := ux.JwtUtil.SignAuthToken(ux.AuthTokenClaims{UserId: 1}, jwtSecret, time.Hour)
	if err != nil {
		t.Fatalf("SignAuthToken error = %v", err)
	...
	req := httptest.NewRequest(http.MethodPost, "/api/task/stopClaim", strings.NewReader("{"))
	req.Header.Set("Authorization", "Bearer "+token)
	res := httptest.NewRecorder()
...
func TestRegisterRoutesRecoversBusinessPanic(t *testing.T) {
	...
	want := `{"code":400,"message":"family name is required"}`
	if strings.TrimSpace(res.Body.String()) != want {
		t.Fatalf("body = %s, want %s", res.Body.String(), want)
	}
}
